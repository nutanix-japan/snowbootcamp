...
Having Prism send key alerts to ServiceNow opens up a world of additional IT Operations Management (ITOM) workflows for applications running on Nutanix infrastructure - but can it deliver on automated remediation based on those alerts?

.. figure:: images/yup.gif

*But lab guide! Can't Prism X-Play do that too?!*

Great point, astute reader! X-Play makes easy work of automating datacenter operations, but combining it with ServiceNow (similar to the Calm Plugin) provides added visibility, and an enterprise-grade rules engine to process data. Prism now supports launching X-Play Playbooks via webhook, making this integration possible.
...
#. Click :fa:`plus` **Add Action** and select **VM Add Memory**.
...
#. In the *Filter Navigator* field in the upper left-hand corner, return to **All Events**.
...
Similar to a Playbook, Flows are processes consisting of a sequence of actions and a trigger, designed to automate business logic for a particular application or process. This Flow will process Alert data, and pass variables to an Action responsible for issuing a REST API call to trigger your Playbook webhook.

Building the Action
-------------------

#. In the *Filter Navigator* field in the upper left-hand corner, search for and then click on **Studio**. The Studio will open in a new tab.
...
#. In the upper left-hand corner of **Studio**, click :fa:`plus` **Create Application File**.
...
#. Click :fa:`plus` **Create Input** three times to create three input variables for your Action.
...
#. Under **Request Details**, fill out the following:
   ...
   .. note::
      ...
      Double check you are creating the above under **Headers** and not **Query Parameters**!
...
#. Replace **<ENTITY_TYPE>**, **<ENTITY_NAME>**, and **<ENTITY_UUID>** with the appropriate **Input Variables** by dragging and dropping from the **Data** column.
   ...
   .. note::
      ...
      Additionally, the *entity1* line in the *Request Body* should be on a single line.

   .. figure:: images/17.png

   .. raw:: html

      <video width="800" preload="auto" autoplay muted controls src="_static/video/SNOW_webhook_API_demo.mp4"> type="video/mp4" </video>
...
#. Click **Run Test**, followed by **Your test has finished running. View the action execution details.**.
...
#. In **Flow Designer**, select the **Home** tab, and then click :fa:`plus` **New > Flow**.
...
#. Fill out the following fields:
   ...
   - Select :fa:`plus` **Add Filters**
   - Select **Resource** from **--choose field--** drop-down menu
   - Select **starts with**
   - Specify your *User##*\ 
   - Click **AND**
   - Select **Metric Name**
...
#. Under **Actions**, select the :fa:`plus` icon.

#. Click **Action**, and type **calm**. Select your previously published *User##*\ -RESTAPI Action.
...
#. Select your existing **User**\ *##* \ **-VM Memory Usage** alert, and then click **Resolve** to clear this alert, before we generate a new alert.

#. SSH into your *Initials*\ **-CentOS VM as **root**, and then run the following commands to begin consuming free memory:
